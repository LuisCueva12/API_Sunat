...
- Integración Greenter (generación+firma XML UBL, envío SOAP, parseo CDR) — **solo para Factura y Boleta**, ambas verificadas en vivo contra SUNAT BETA con estado `ACEPTADO` (`MapeadorFacturaBoletaGreenter` mapea ambas al mismo `Invoice` de Greenter, con `tipoDoc` 01/03 según catálogo 1 SUNAT). Nota de Crédito y Nota de Débito tienen el dominio completo (validadores, catálogo de motivos, máquina de estados) pero `GeneradorXmlFirmadoGreenter` todavía no sabe generar su XML (`Note` de Greenter) — cualquier intento de emitirlas queda en `ERROR` con el mensaje "aún no soportado por GeneradorXmlFirmadoGreenter". No es un bug latente: se descubrió y confirmó en vivo el 2026-08-19, ver Registro.
...
- Suite completa verificada con PostgreSQL 18, Redis 7 y `ext-soap`: **137 tests, 325 assertions, 0 fallos, 0 omitidos**. Incluye Unit, Integration, Feature, seguridad del panel y concurrencia real de correlativos.
...
- Nota de Crédito y Nota de Débito no se pueden emitir de verdad todavía: falta el mapeo a `Greenter\Model\Sale\Note` (con `NoteBuilder`) en `GeneradorXmlFirmadoGreenter`. El dominio, la validación del catálogo de motivos y las pruebas ya están listos para cuando se implemente.
...
1. Mapear Nota de Crédito y Nota de Débito a `Greenter\Model\Sale\Note` y verificarlas en vivo contra SUNAT BETA, igual que se hizo con Factura y Boleta.
2. Completar Fase 8 en el panel: importación/rotación segura de certificado, credenciales SUNAT y API Keys.
3. Registrar los envíos a SUNAT y los eventos del pipeline en `envios_sunat`/`eventos_comprobante` (hoy el pipeline no persiste ahí; ver gap arriba).
4. Resolver las reglas tributarias pendientes de `docs/05_SUNAT.md` (incluida la Comunicación de Baja) antes de promover cada tipo de comprobante a producción.
