...
9. **Interfaces web** — `/admin` (Filament interno), `/gestion` (Filament operativo de respaldo) y `/app` (facturador Blade propio para la empresa), sobre los mismos casos de uso.
...
**Ya no está fuera de alcance** (decisión tomada 2026-08-19, ver §14): portal self-service por empresa (`/app`, interfaz Blade propia separada del panel interno `/admin`) — pasó de "fuera de alcance V1" a parte implementada del MVP.
...
**Resuelta e implementada 2026-08-19** (ver §14): la interfaz V1 deja de ser exclusivamente interna — facturador self-service propio (`/app`) construido y verificado con aislamiento multiempresa, además de `/admin`. El antiguo Filament de empresa vive en `/gestion` como respaldo. Los roles granulares siguen pendientes.
...
- **Facturador `/app` self-service por empresa**: interfaz Laravel Blade propia con sesión `web` y middleware `AsegurarUsuarioFacturador`; exige `empresa_id !== null`. Bootstrap solo aporta layout/responsive, mientras CSS propio, Alpine.js, Lucide e Inter definen la experiencia. Las consultas de comprobantes, clientes y productos se restringen por empresa antes de resolver IDs o búsquedas. La emisión invoca `EmitirFactura`/`EmitirBoleta`, nunca reproduce la lógica tributaria en la vista. El antiguo `EmpresaPanelProvider` permanece en `/gestion` como respaldo operativo y comparte el mismo guard; `/admin` sigue exigiendo usuario sin empresa + `super_admin`.
- **Catálogo de productos/servicios**: tabla/modelo Eloquent liviano por tenant (`productos_servicios`) dentro de Interfaces, deliberadamente sin nuevo agregado de dominio porque solo es una ayuda de captura de nombre, unidad y precio. No gestiona inventario, compras ni contabilidad. Nueva venta puede buscarlo o usar una línea manual.
...
1. Roles granulares dentro de una empresa (`facturador`/`contador`/`empleado`/`empresa_admin`) cuando haya una necesidad real de diferenciar permisos — hoy cualquier usuario con `empresa_id` ve todo lo de su empresa en `/app`.
2. Escritor de auditoría (la tabla `auditorias` ya tiene el esquema correcto — usuario/integración/empresa/acción/entidad/ip/request_id/datos previos-nuevos — pero ningún caso de uso escribe ahí todavía).
3. Envelope de respuesta API: agregar campo `"success": true/false` (hoy es `{"data"/"error", "meta"}` sin ese campo) — cambio de contrato aceptable ahora porque todavía no hay integraciones reales en producción.
4. Rate limiting diferenciado por tipo de endpoint (login, emisión de token, emisión de comprobante, consulta) — hoy es un único límite genérico de 60/min por IP.
