...
Expuesto en `/admin`, en el respaldo Filament `/gestion` y en el facturador Blade `/app`, siempre con `empresa_id` resuelto del usuario y nunca desde un campo manipulable. Está conectado a la emisión: el usuario puede buscar por documento o nombre para autocompletar, escribir el receptor directamente o emitir una Boleta sin documento. La API también puede resolver una razón social omitida por empresa + documento. En todos los casos el snapshot desnormalizado en `comprobantes.receptor_*` sigue siendo la fuente de verdad fiscal.
