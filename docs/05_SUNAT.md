...
- [ ] Comunicación de Baja de boletas mediante resumen diario (`sendSummary`). La regla de plazo ya fue confirmada en la página oficial vigente de [Boleta de Venta Electrónica](https://cpe.sunat.gob.pe/tipos_de_comprobantes/boleta): debe corresponder al día de emisión y puede enviarse, según el caso, hasta el sétimo día calendario posterior a la CDR aceptada o a la generación. Greenter dispone de `VoidedBuilder` y consulta asíncrona mediante ticket, pero esta vertical no forma parte del alcance V1 actual y no está implementada.
