...
- **Certificado digital**: `CrearCertificadoDigital` exige que el PEM sea un X.509 válido y no esté vencido (`AnalizadorCertificadoDigital`, ver [02_DOMINIO.md](02_DOMINIO.md)) antes de cifrarlo. Solo puede existir un certificado `ACTIVO` por empresa — índice único parcial en Postgres, reforzado en el caso de uso: si ya hay uno activo, se marca `REEMPLAZADO` (nunca se borra, queda como histórico auditable) dentro de la misma transacción antes de insertar el nuevo. El PEM y el password se cifran vía el cast `encrypted` de Eloquent (mismo mecanismo que credenciales SUNAT), nunca en texto plano en BD.
