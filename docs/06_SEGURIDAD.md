...
- **Multiempresa**: `empresa_id` **siempre** derivado del actor autenticado (cliente OAuth vía `owner_id`, o sesión de usuario en el panel), nunca aceptado como campo de request — evita IDOR entre tenants. El vínculo integración→empresa usa `oauth_clients.owner_type`/`owner_id` nativos de Passport (retipado a `uuid` en migración, Passport lo crea `bigint` por defecto) apuntando a `App\Models\Empresa`, sin tabla propia adicional. Tests explícitos de fuga entre empresas son no negociables (ver [01_ARQUITECTURA.md](01_ARQUITECTURA.md) Fase 6).
